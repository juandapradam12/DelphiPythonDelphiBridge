unit CylinderProcessor32;

{
  CylinderProcessor32 - 32-bit version calling main.py exactly like 64-bit
  
  Exact copy of CylinderProcessor64.pas but using PyEngineService32
  for 32-bit Python integration
}
...
uses
  ...
  PythonEngine, VarPyth, PyEngineService32;

function ProcessCylinderFile32(const FilePath: string): string;
function LoadFileAndConvertToBytes32(const FileName: string): TBytes;
...
begin
  // Exact copy of LoadFileAndConvertToBytes64
...
begin
  // Exact copy of ProcessCylinderFile64 logic but using 32-bit Python
  ...
  try
    ...
    RawBytes := LoadFileAndConvertToBytes32(FilePath);

    // Initialize Python environment - use 32-bit service
    PyEngine32.EnsureReady;
